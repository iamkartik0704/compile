 ...
 * No AI needed.
 */
export function instrumentCppSolution(userCode) {
...
function detectVarDeclaration(line) {
  ...
  // auto result = ...;
  const patterns = [
    // Type name = expr;  or  Type name;
    /^(int|long|long long|double|float|bool|char|string|auto)\s+([a-zA-Z_]\w*)\s*[=;]/,
    // Container<...> name (with or without space before name)
    // Handle nested templates: vector<pair<int,int>>car(n);
    /^((?:vector|stack|queue|priority_queue|deque|list|set|unordered_set|map|unordered_map|pair)\s*<.+?>)\s*([a-zA-Z_]\w*)\s*[\(=;{]/,
    // TreeNode* name
    /^(TreeNode\s*\*|ListNode\s*\*)\s*([a-zA-Z_]\w*)\s*[=;]/,
  ]
...
/**
 * Build a C++ expression string that produces JSON for scalar variables.
 * E.g.: "{\"n\":" + std::to_string(n) + ",\"i\":" + std::to_string(i) + "}"
 */
function buildVarsJson(varNames) {
  ...
  const parts = vars.map((name, idx) => {
    const prefix = idx > 0 ? ',' : '{'
    // Produces C++ string: ",\"name\":" or "{\"name\":"
    return '"' + prefix + '\\"' + name + '\\":" + dsa_toJson(' + name + ')'
  })
  return parts.join(' + ') + ' + "}"'
}

/**
 * Build a C++ expression string for the dataStructureState field.
...
function isContainerType(type) {
  ...
  const t = type.replace(/\bstd::/g, '').replace(/\bconst\b/g, '').trim()
  return /^(vector|stack|queue|priority_queue|deque|list|set|unordered_set|map|unordered_map|pair)\s*</.test(t)
    || t === 'TreeNode*' || t === 'ListNode*'
}
